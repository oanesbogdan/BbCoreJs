    ...
    ContentRenderer = new JS.Class({
        ...
        initialize: function () {
            this.templates = {
                ...
                grid: require('text!cs-templates/content.list.view.tpl'), //no difference
        ...
        bindContentEvents: function (item, itemData) {
            item = $(item);
            item.on('click', ".show-content-btn",$.proxy(this.showContentPreview, this, itemData));
            item.on('click', ".del-content-btn", $.proxy(this.deleteContent, this, itemData));
            return item;
        },
        ...
        deleteContent: function (itemData) {
            ...
            jQuery.ajax({
                ...
                data = data || [];
                var templateData = {isOrphaned : (data.length==0), items: data };
                var content = nunjucks.renderString(self.templates.deleteContent, templateData);
                self.popin.unmask();
        ...
        addButtons : function () {
            ...
            self.popin.addButton("No", function () {  self.popin.hide(); });
        }
